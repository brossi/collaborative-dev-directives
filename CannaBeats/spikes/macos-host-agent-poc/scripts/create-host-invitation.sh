...
if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "Usage: $0 \"Display name\" [hours]" >&2
  exit 2
fi
...
ssh "$deploy_host" bash -s -- "$hours" "$encoded_name" <<'REMOTE'
set -euo pipefail
hours="$1"
display_name="$(printf '%s' "$2" | base64 --decode)"
cd /opt/cannabeats-poc
docker compose exec -T app node cli.mjs invite \
  --role host \
  --hours "$hours" \
  --note "$display_name"
REMOTE
