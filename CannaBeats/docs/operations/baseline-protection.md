...
`release.sh`, `rollback-release.sh`, and the scheduled coordinated backup take the same exclusive host-operation lock before inspecting or changing release/admission state. They reject concurrent operation rather than racing admission or the rollback floor. Release rejects reused application identities and source/deployed catalog drift, then compares the live database `user_version` with the candidate's checked-in minimum, maximum, and target contract. It also proves that the current image can read the candidate's migration target before deployment, so rollback is possible after startup migrations.

Before the first managed release it records the exact image IDs and release environment of the running access/game containers as the bootstrap rollback target. Existing P1 `current`, `previous`, and used-version files are adopted without losing their identities. The script then performs an encrypted verified backup, validates Compose, builds the images, resolves their exact immutable image IDs, closes admission, and stops app/game. It durably selects the candidate release before exposing its app/game containers, requires readiness and schema checks, then reopens admission. A hard interruption therefore leaves either the old record with app/game stopped or the new record with matching containers (or stopped); it cannot leave replacement containers serving beneath the old record.
...
The rollback obtains the same exclusive lock, reads the live database version, and refuses to select a previous image whose recorded range cannot read it. It closes admission and stops app/game, atomically selects the rollback release, then starts exactly the selected images, checks readiness/schema access, and reopens admission only after success. No automatic down-migration is attempted. A future schema target outside the current image's readable range is rejected during forward release and requires a separately designed expand/contract migration.

On Linux the operation fence uses a process-bound `flock`, which is released automatically when the process dies. The checked-in portable fallback records PID plus process-start identity and reclaims only a demonstrably stale directory. Do not remove or replace the lock while an operation is alive. After an interruption, run `sudo CANNABEATS_COMPOSE_DIR=/opt/cannabeats/CannaBeats/spikes/access-spotify-poc deploy/reconcile-release.sh`; it starts the durable `current` record, verifies readiness and State epoch, and only then opens admission.
...
| Authorized component status | `docker compose -f compose.yaml -f compose.state-cutover.yaml --profile operations run --rm operator operator-status --format json --fail-on unavailable` | The bounded one-shot operator combines Access/game readiness, database integrity/volume, authenticated relay, certificate, State validation, and State-owned source status without giving the long-running app the operator token |
| Authorized session summary | `docker compose -f compose.yaml -f compose.state-cutover.yaml --profile operations run --rm operator operator-summary --since-hours 24` | Current/recent State snapshots, counts, safe presence facts, audio lease/command state, and conservative liveness; no writes or names |
...
| Managed-source token | Source node file; SHA-256 only in SQLite | Stop the controller, run the bounded operator service with `managed-source rotate --source-id UUID`, install the once-shown token as root-owned mode `0400`, restart controller, confirm heartbeat; rotation releases an active lease |
...
1. Disable the lost source with `docker compose -f compose.yaml -f compose.state-cutover.yaml --profile operations run --rm operator managed-source disable --source-id UUID`. Revoke its tailnet and provider access. If compromise is possible, rotate the relay ingest token and revoke the Spotify authorization.
...
3. Copy the named release of `spikes/managed-audio-source-poc`, run `sudo infra/install-runtime.sh`, and install the separately built `btaudio-learning` runtime at the exact commit recorded in `spikes/access-spotify-poc/deploy/btaudio-runtime.version` under `/opt/btaudio-venv`. Install `spikes/access-spotify-poc/deploy/btaudio-relay-generation-fence.conf` as the `btaudio-relay.service` override on the relay host; `systemctl cat btaudio-relay.service` must show `--disconnect-listeners-on-source-disconnect`. Without that flag, a listener can survive into the next publisher generation and the cross-lobby isolation gate is not satisfied. The runtime installer leaves VNC disabled: create a temporary password file interactively with `x11vnc -storepasswd`, run `sudo /opt/cannabeats-managed-source/install-vnc-password.sh "$HOME/.vnc/passwd"`, then delete the temporary copy.
4. Register a new source with `docker compose -f compose.yaml -f compose.state-cutover.yaml --profile operations run --rm operator managed-source register --name NAME`. Transfer the once-shown source token and relay ingest token out of band. Install them with the owners and modes described in the source README.
